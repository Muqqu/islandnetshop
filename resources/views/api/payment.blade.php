                    ...
                    <div class="peymentToggler">
                        <div class="paymentHeader showHide">
                            ...
                            <div class="sg-card">
                                ...
                                <ul class="global-list">
                                    <li>{{ __('subtotal') }}<span>{{ get_price($orders->sum('sub_total'),$currency) }}</span>
                                    </li>
                                    <li>{{ __('tax') }} <span>{{ get_price($orders->sum('total_tax'),$currency) }}</span>
                                    </li>
                                    <li>{{ __('discount') }}<span>{{ get_price($orders->sum('discount'),$currency) }}</span></li>
                                    <li>{{ __('shipping_cost') }}
                        ...
                        <div class="paymentBTN-group">
                            <div class="order-total sm-fixed-bottom">
                                <p>{{ __('total') }}<span id="iconHi"><span class="icon mdi mdi-name mdi-chevron-down"></span></span> <span>{{ get_price($orders->sum('total_payable'),$currency) }}</span></p>
                                
                                <a href="javascript:void(0)" class="btn btn-primary paymentBTNFixed disable_btn">{{ __('pay_now') }}</a>
                                
                                <div class="div_btns d-none">
                                    <a href="{{ url("stripe/redirect?trx_id=$trx_id&payment_mode=api&code=$code&token=$token")  }}" class="btn btn-primary paymentBTNFixed payment_btns d-none stripe_btn"> {{ __('pay_now') }}</a>
